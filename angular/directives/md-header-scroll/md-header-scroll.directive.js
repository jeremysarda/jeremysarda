class MdHeaderScrollController {
    constructor($window) {
        'ngInject';
...
MdHeaderScrollDirective.$inject = ['$window'];
export function MdHeaderScrollDirective($window) {
    return {
        restrict: 'A',
        scope: {ngModel: '=ngModel'},
        controller: MdHeaderScrollController,
        link: function (scope, element) { //, attrs, controllers) {

            /* header DOM element with md-page-header attribute */
            let header = document.querySelector('[md-page-header]');
            ...
            let title = angular.element(document.querySelector('[md-header-title]'));
            /* DOM element with md-header-picture attribute (picture in header) */
            let picture = angular.element(document.querySelector('[md-header-picture]'));
            /* DOM element with main-fab class (a DOM element which contains the main float action button element) */
            let fab = angular.element(document.querySelector('.main-fab'));
            ...
            angular.element($window).bind("scroll", function () {
                let dimensions = header.getBoundingClientRect();
